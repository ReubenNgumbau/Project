            ...
            <?php if(isset($_SESSION['user_id'])): ?>
                <div class="dropdown">
                    <button class="btn btn-secondary dropdown-toggle bg-transparent  text-light border-0" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
                        <span class="me-1"><img src="<?php echo is_file('./uploads/avatars/'.$_SESSION['user_id'].'.png') ? './uploads/avatars/'.$_SESSION['user_id'].'.png?v='.(is_null($_SESSION['date_updated']) ? strtotime($_SESSION['date_created']) : strtotime($_SESSION['date_updated'])) : './images/no-image-available.png' ?>" alt="avatar" class="avatar rounded-circle"></span>    Hello <?php echo $_SESSION['name'] ?>
                    </button>
