    ...
    <div class="col-12 flex-grow-1">
        <div class="row h-100">
            <div class="col-md-4 h-100 d-flex flex-column">
                ...
                <div class="card shadow-lg rounded-0 flex-grow-1">
                    ...
                    <div class="list-group" id="convo_list">
                        <?php 
                        if(isset($_GET['eid'])){
                            $conn->query("UPDATE `messages` set status = 1 where to_user = '{$_SESSION['name']}' and md5(`from_user`) = '{$_GET['eid']}' ");
                        }
                        $convo_qry = $conn->query("SELECT *, concat(full_name) as name FROM consultants where user_id in (SELECT to_user FROM messages where from_user = '{$_SESSION['name']}') or user_id in (SELECT from_user FROM messages where to_user = '{$_SESSION['name']}')");

                        $convo_list_arr = array();
                        while($row=$convo_qry->fetch_assoc()):
                            $last_message_qry = $conn->query("SELECT * FROM messages where (from_user = '{$_SESSION['user_id']}' and to_user = '{$row['user_id']}') or (to_user = '{$_SESSION['name']}' and from_user = '{$row['user_id']}') order by unix_timestamp(`date_created`) desc limit 1");
                            ...
                            $un_read = $conn->query("SELECT * FROM messages where to_user = '{$_SESSION['name']}' and from_user = '{$row['user_id']}' and status = '0' ")->num_rows;
                             $row['un_read'] = $un_read > 0 ? $un_read : '';
                            $convo_list_arr[strtotime($row['last_message_created'])] = $row;
                        endwhile;
                        $conn->query("UPDATE messages set popped = 1 where to_user = '{$_SESSION['name']}' ");
                        krsort($convo_list_arr);
                        ?>
                        <?php foreach($convo_list_arr as $row): ?>
                            <a href="./?eid=<?php echo md5($row['user_id']) ?>" class="list-group-item list-group-item-action list-item rounded-0 convo_with" data-id = '<?php echo $row['user_id'] ?>'>
                                <div class="w-100 d-flex position-relative">
                                    <div class="col-2 px-1">
                                        <img src="<?php echo is_file('./uploads/avatars/'.$row['user_id'].'.png') ? './uploads/avatars/'.$row['user_id'].'.png?v='.(!is_null($row['date_updated']) ? strtotime($row['date_updated']) : strtotime($row['date_created'])) : './images/no-image-available.png' ?>" class='user-search-avatar rounded-circle bg-light border border-dark' alt="">
                                    ...
                                    <div class="col-10 lh-1">
                                        ...
                                        <div class="text-muted search-user-email text-truncate last-message-field" title="<?php echo $row['last_message'] ?>"><?php echo ($row['last_message_id'] == $_SESSION['name'] ? 'You: ':'').$row['last_message'] ?></div>
                                    </div>
                                </div>
                            </a>
                        <?php endforeach; ?>
            ...
            <div class="col-md-8 h-100" id="right-panel">
                ...
                $qry = $conn->query("SELECT *,CONCAT(full_name,') as full_name FROM  `consultants` where md5(user_id) = '{$eid}' ");
                if($qry->num_rows > 0):
                    foreach($qry->fetch_array() as $k => $v){
                        $user_to[$k] = $v;
                    }
                $convo_with = $user_to['user_id'];
                $messages = $conn->query("SELECT * FROM `messages` where (from_user = '{$_SESSION['user_id']}' and to_user = '{$user_to['user_id']}') OR (to_user = '{$_SESSION['name']}' and from_user = '{$user_to['user_id']}') order by unix_timestamp(date_created) desc limit {$message_limit} offset {$message_offset}");
                while($row=$messages->fetch_assoc()){
                    ...
                    $messages_ids[] = $row['user_id'];
                }
                $user_to_avatar = is_file('./uploads/avatars/'.$user_to['user_id'].'.png') ? './uploads/avatars/'.$user_to['user_id'].'.png?v='.(!is_null($user_to['date_updated']) ? strtotime($user_to['date_updated']) : strtotime($user_to['date_created'])) : './images/no-image-available.png';
