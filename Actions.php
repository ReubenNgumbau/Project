...
Class Actions extends DBConnection{
    ...
    function login(){
        extract($_POST);
        $sql = "SELECT * FROM consultants where email = '{$email}' and `password` = '".md5($password)."' ";
    ...
    function save_user(){
        ...
        @$check= $this->conn->query("SELECT * FROM consultants where `email` = '{$email}' ".($id > 0 ? " and id != '{$id}' " : ""))->num_rows;
        if(@$check> 0){
            $resp['status'] = 'failed';
            $resp['msg'] = "Email already exists.";
        }else{
            if(empty($id)){
                $sql = "INSERT INTO `consultants` set {$data}";
            }else{
                $sql = "UPDATE `consultants` set {$data} where user_id = '{$id}'";
            }
    ...
    function delete_user(){
        ...
        @$delete = $this->conn->query("DELETE FROM `consultants` where id = '{$id}'");
        if($delete){
            $resp['status']='success';
    ...
    function update_credentials(){
        ...
        if(!empty($password) && md5($old_password) != $_SESSION['password']){
            ...
            $sql = "UPDATE `consultants` set {$data} where id = '{$_SESSION['name']}'";
            @$save = $this->conn->query($sql);
            if($save){
                $resp['status'] = 'success';
                $_SESSION['flashdata']['type'] = 'success';
    ...
    function find_user(){
        extract($_POST);
        $sql = "SELECT *,CONCAT(full_name,' ) as `name` FROM `consultants` where (CONCAT(full_name,') LIKE '%{$keyword}%' OR CONCAT(full_name,') LIKE '%{$keyword}%' OR email LIKE '%{$keyword}%') and user_id != '{$_SESSION['name']}'";
        $search = $this->conn->query($sql);
        $data = array();
        while($row = $search->fetch_assoc()){
            $row['avatar'] = is_file('./uploads/avatars/'.$row['id'].'.png') ? './uploads/avatars/'.$row['user_id'].'.png?v='.(!is_null($row['date_updated']) ? strtotime($row['date_updated']) : strtotime($row['date_created'])) : './images/no-image-available.png';
            $row['user_id'] = md5($row['user_id']);
            $data[] = $row;
        }
    ...
    function get_messages(){
        ...
        $messages = $this->conn->query("SELECT * FROM `messages` where ((from_user = '{$_SESSION['user_id']}' and to_user = '{$convo_with}') OR (to_user = '{$_SESSION['user_id']}' and from_user = '{$convo_with}')) and user_id > {$last_id} order by unix_timestamp(date_created) asc ");
        $data = array();
        $ids = array();
        ...
        if(count($ids) > 0)
        $this->conn->query("UPDATE `messages` set status = 1 where user_id in (".implode(',',$ids).") ");
